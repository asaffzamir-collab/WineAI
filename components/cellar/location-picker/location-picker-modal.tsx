      ...
      <DialogContent onClose={onClose} className="max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="space-y-4">
                  ...
                  {Array.from({ length: shelf.heightRows }).map((_, rowIdx) => {
                    ...
                    return (
                      <div
                        ...
                        style={{ gridTemplateColumns: `repeat(${activeRack.columns}, minmax(32px, 1fr))` }}
                      >
                        {Array.from({ length: activeRack.columns }).map((_, col) => {
                          ...
                          return (
                            <button
                              ...
                              className={cn(
                                'aspect-[3/5] rounded-md transition-all relative',
                                isEmpty
                              ...
                              {isEmpty ? (
                                <div className="absolute inset-0 flex items-center justify-center">
                                  {isSelected ? (
                                    ...
                                    <Plus className="h-3 w-3 text-muted-foreground/50" strokeWidth={1.5} />
                                  )}
                                </div>
                              ) : placement ? (
          ...
          <div className="flex gap-2">
            ...
            <Button className="flex-1" onClick={handleConfirm} disabled={!selectedSlot}>
              {t('pickerConfirm')}
            </Button>
          </div>
        </div>
