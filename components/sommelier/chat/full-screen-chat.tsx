...
export function FullScreenChat({ conversationId, onBack, initialSidebarOpen }: FullScreenChatProps) {
  ...
  const chatHeader = (
    <>
      <div className="pt-[env(safe-area-inset-top)] flex-shrink-0 md:hidden" />
  ...
  const chatMessages = (
    <div className="flex-1 overflow-y-auto min-h-0 overscroll-none">
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-full px-6 pb-4 pt-12">
      ...
      <div className="max-w-2xl mx-auto px-3 py-2">
        ...
        >
          <textarea
            ...
            enterKeyHint="send"
            className={cn(
              'flex-1 resize-none rounded-xl border border-border/60 bg-background px-3 py-2.5 text-base',
  ...
  return (
    <>
      {/* Mobile: normal document flow (no position:fixed) */}
      <div className="h-[100dvh] flex flex-col bg-background md:hidden">
        {historySidebar}
        {chatHeader}
        {chatMessages}
        {chatInput}
      ...
      <div className="hidden md:flex fixed inset-0 z-[70] flex-col bg-background overflow-hidden">
        {historySidebar}
        {chatHeader}
        {chatMessages}
        {chatInput}
